...
from .efficientnet_utils import efficientnet_params
import sys
...
class AsppModule(nn.Module):
    ...
    def forward(self, input):
        ...
        input5 = F.upsample(input5, input4.size()[2:], mode='bilinear')
...
class EfficientAsppModule(nn.Module):
    def __init__(self, comp_coeff, bn_momentum=0.1, output_stride=16):
        super(EfficientAsppModule, self).__init__()
        self.efficientnet_name = 'efficientnet-b' + str(comp_coeff)
        w, d, r, _ = efficientnet_params(self.efficientnet_name)
        self.filters = int(1280 * w)
        # output_stride choice
        if output_stride == 16:
            atrous_rates = [0, 6, 12, 18]
        elif output_stride == 8:
            ...
            raise Warning("output_stride must be 8 or 16!")
        # atrous_spatial_pyramid_pooling part
        self._atrous_convolution1 = _AsppConv(self.filters, 256, 1, 1, bn_momentum=bn_momentum)
        self._atrous_convolution2 = _AsppConv(self.filters, 256, 3, 1, padding=atrous_rates[1], dilation=atrous_rates[1]
                                              , bn_momentum=bn_momentum)
        self._atrous_convolution3 = _AsppConv(self.filters, 256, 3, 1, padding=atrous_rates[2], dilation=atrous_rates[2]
                                              , bn_momentum=bn_momentum)
        self._atrous_convolution4 = _AsppConv(self.filters, 256, 3, 1, padding=atrous_rates[3], dilation=atrous_rates[3]
                                              , bn_momentum=bn_momentum)

        # image_pooling part
        self._image_pool = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),
            nn.Conv2d(self.filters, 256, kernel_size=1, bias=False),
            SynchronizedBatchNorm2d(256, momentum=bn_momentum),
            nn.ReLU()
        )
...
class EfficientEncoder(nn.Module):
    def __init__(self, comp_coeff, bn_momentum=0.1, output_stride=16):
        ...
        self.ASPP = EfficientAsppModule(comp_coeff=comp_coeff, bn_momentum=bn_momentum, output_stride=output_stride)
        self.relu = nn.ReLU()
        self.conv1 = nn.Conv2d(1280, 256, 1, bias=False)
        self.bn1 = SynchronizedBatchNorm2d(256, momentum=bn_momentum)
        self.dropout = nn.Dropout(0.5)
