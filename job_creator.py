...
SRUN_COMMAND = '$python -u /homes/mcipriano/projects/alveolar_canal_2D_CAE/main.py --base_config '
EXPERIMENT_DIR = '/nas/softechict-nas-2/mcipriano/results/maxillo/3D/'
...
if __name__ == '__main__':
    ...
    for i, r in df[df.todo.eq(True)].iterrows():
        ...
        if 'num_labels' in r.keys():
            if r['num_labels'] == 2:
                config['data-loader']['labels'] = binary_labels
                config['data-loader']['weights'] = [7.03e-04, 9.99e-01]
            else:
                config['data-loader']['labels'] = multi_labels
                config['data-loader']['weights'] = [4.5132e-01, 5.4842e-01, 2.5372e-04]
        ...
        project_dir = path.join(EXPERIMENT_DIR, title)
